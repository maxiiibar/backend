...
describe("TEST API REST", () => {
  ...
  test("[POST] /users/register", async () => {
    ...
    assert.ok(
      responseInvalidJson.data,
      "Debería contener un mensaje de error indicando que el correo es inválido."
    );
    assert.ok(
      responseInvalidJson.data.includes("Invalid email format."),
      "El mensaje de error debería indicar que el correo electrónico es inválido"
    );
    ...
    assert.ok(
      responseDuplicateJson.data,
      "Debería devolver un mensaje de error para usuarios duplicados."
    );
    assert.ok(
      responseDuplicateJson.data.includes("Email already registered."),
      "El mensaje de error debería indicar que el email ya está registrado."
    );
  });
  test("[POST] /products", async () => {
    ...
    assert.equal(
      response.status,
      200,
      "El producto debería agregarse correctamente"
    );
    assert.ok(responseJson.data._id, "El producto debería tener un id");
    assert.equal(
      typeof responseJson.data._id,
      "string",
      "El id debería ser una cadena de texto"
    );
    assert.equal(
    ...
    assert.equal(
      responseJson.data.price,
      body.price,
      "El precio debería coincidir"
    );
    assert.equal(
      responseJson.data.stock,
      body.stock,
      "El stock debería coincidir"
    );
    assert.equal(
      responseJson.data.category,
      body.category,
      "La categoría debería coincidir"
    );
  ...
  test("[POST] /carts/product/:idProd", async () => {
    ...
    assert.equal(
      responseJson.status,
      401,
      "El producto no debería agregarse correctamente"
    );
    assert.ok(
      responseJson.error,
      "Debería contener un mensaje de error que indique que no se puede agregar productos propios al carrito."
    );
    assert.ok(
      responseJson.error.includes("You can't add your own products."),
      "El mensaje de error debería indicar que no se puede agregar productos propios al carrito."
    );
  });
});
